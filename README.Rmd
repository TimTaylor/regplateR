...
| Function                         | returns                                               |
|----------------------------------|-------------------------------------------------------|
| `isCurrentReg(x)`                | index of plates in `x` in the current format          |
| `isPrefixReg(x)`                 | index of plates in `x` in the prefix format           |
| `isSuffixReg(x)`                 | index of plates in `x` in the suffix format           |
| `isDatelessReg(x)`               | index of plates in `x` in the dateless format         |
| `isValidReg(x)`                  | index of plates in `x` in the 1 of above formats      |
| `regDate(x)`                     | earliest possible registration dates of plates in `x` |
